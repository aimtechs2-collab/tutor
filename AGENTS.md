# AIMTutor — Agent-Native Architecture

## Overview

AIMTutor is an **agent-native** intelligent learning companion organized
...
`aimtutor/capabilities/_shared.py` so every turn emits the same envelope
...
```bash
# Install
pip install aimtutor      # Full app (CLI + Web/API + packaged Web assets)
pip install aimtutor-cli  # CLI-only

# Run any capability
aimtutor run chat "Explain Fourier transform"
aimtutor run deep_solve "Solve x^2=4" -t rag --kb my-kb
aimtutor run auto "Animate sine wave"   # picks the right capability
...
| Path                                       | Purpose                              |
| ------------------------------------------ | ------------------------------------ |
| `aimtutor/runtime/orchestrator.py`        | `ChatOrchestrator` — unified entry   |
| `aimtutor/runtime/launcher.py`            | Backend + frontend lifecycle / port discovery |
| `aimtutor/runtime/registry/`              | Tool + Capability registries         |
| `aimtutor/runtime/bootstrap/builtin_capabilities.py` | Built-in capability class paths |
| `aimtutor/services/config/runtime_settings.py` | JSON settings + process-env overrides |
| `aimtutor/core/stream.py`, `stream_bus.py` | StreamEvent protocol + async fan-out |
| `aimtutor/core/tool_protocol.py`          | `BaseTool` + `ToolDefinition`         |
| `aimtutor/core/capability_protocol.py`    | `BaseCapability` + `CapabilityManifest` |
| `aimtutor/core/context.py`                | `UnifiedContext` dataclass            |
| `aimtutor/tools/builtin/__init__.py`      | All built-in tool wrappers           |
| `aimtutor/capabilities/`                  | Built-in capability implementations  |
| `aimtutor_cli/main.py`                    | Typer CLI entry point                |
| `aimtutor/api/routers/unified_ws.py`      | Unified WebSocket endpoint           |
...
```
pip install aimtutor      — Full app (CLI + Web/API + packaged Web assets)
pip install aimtutor-cli  — CLI-only (LLM + RAG + providers + document parsing)
...
.[math-animator]  — Manim addon (powers `visualize` Manim renders + `aimtutor animate`)
