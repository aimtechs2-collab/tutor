"""CLI entry point for the standalone ``aimtutor-cli`` package."""
...
from aimtutor.runtime.mode import RunMode, set_mode
...
app = typer.Typer(
    name="aimtutor",
    help="AIMTutor CLI – agent-first interface for capabilities, tools, and knowledge.",
...
def run_capability(
    ...
    from aimtutor.app import AIMTutorApp
    ...
    maybe_run(run_turn_and_render(app=AIMTutorApp(), request=request, fmt=fmt))
...
def start(
    ...
    from aimtutor.runtime.launcher import start as start_web
...
def serve(
    ...
    if port is None:
        from aimtutor.services.setup import get_backend_port
