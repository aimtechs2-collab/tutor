...
``aimtutor init`` walks the user through a four-step wizard (ports → LLM →
...
review panel) lives in :mod:`aimtutor_cli.init_wizard`. This module is
...
from aimtutor.runtime.home import AIMTUTOR_HOME_ENV, get_runtime_home

from . import init_wizard as wiz


def _reset_runtime_singletons() -> None:
    """Drop cached service instances so the new AIMTUTOR_HOME takes effect.

    ``aimtutor init`` may pass ``--home`` to target a different workspace; the
...
def _embedding_step(
    ...
    from aimtutor.services.config.embedding_endpoint import (
...
def run_init(*, cli_only: bool = False, home: str | Path | None = None) -> None:
    ...
    from aimtutor.services.config import get_model_catalog_service, get_runtime_settings_service
