"""Interactive setup helpers used by ``aimtutor init``.
...
from aimtutor.services.provider_registry import PROVIDERS, ProviderSpec, find_by_name
...
# ``aimtutor.services.config.provider_runtime``. Adding a new featured entry
...
# ``aimtutor.services.config.provider_runtime``. Each entry below describes
...
def _all_providers_except(featured: set[str]) -> list[ProviderSpec]:
    ...
    return [
        ...
        if spec.name not in featured and not spec.is_oauth  # OAuth flows use `aimtutor login`
    ]
