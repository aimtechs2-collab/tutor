...
- **Backend** — `SkillService` (`aimtutor/services/skill/service.py`) provides CRUD + listing + selection with strict name validation (`^[a-z0-9][a-z0-9-]{0,63}$`), frontmatter parsing, and directory-level isolation. REST API mounted at `/api/v1/skills` with `GET /list`, `GET /{name}`, `POST /create`, `PUT /{name}`, `DELETE /{name}`.
...
- **`/api/version` route** — a Next.js ISR endpoint that queries `api.github.com/repos/HKUDS/AIMTutor/releases/latest` (hourly revalidation, optional `GITHUB_TOKEN` for rate-limit headroom) and returns the latest tag, name, URL, and publish date.
...
**Full Changelog**: https://github.com/HKUDS/AIMTutor/compare/v1.2.1...v1.2.2
