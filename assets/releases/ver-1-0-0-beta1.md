# 🚀 AIMTutor v1.0.0-beta1 Release Notes

**Release Date:** 2026.04.04

We're thrilled to announce **AIMTutor v1.0.0-beta1** — the first beta of the **AIMTutor 2.0** architecture. This is a ground-up rewrite that transforms AIMTutor from a monolithic RAG tutor into an **agent-native learning platform** with a two-layer plugin model (Tools + Capabilities), three unified entry points (CLI / WebSocket / Python SDK), and a completely rebuilt web application shell.

> ⚠️ **Beta Notice:** This is **beta 1** of v1.0.0. The core architecture is stable, but some **UI interactions and edge-case workflows may still contain bugs**. We appreciate your patience and welcome bug reports via [Issues](https://github.com/HKUDS/AIMTutor/issues).
...
> [!TIP]
> **Call for Feedback:** If you encounter any bugs or have feature requests, please [open an issue](https://github.com/HKUDS/AIMTutor/issues)! PRs are welcome — see our [Contributing Guide](https://github.com/HKUDS/AIMTutor/blob/main/CONTRIBUTING.md).
...
- **Architecture** — Complete rewrite from `src/` to `aimtutor/` + `aimtutor_cli/` with agent-native runtime (Tools + Capabilities).
- **Entry Points** — Three unified entry points: standalone CLI (`aimtutor`), WebSocket API (`/api/v1/ws`), and Python SDK facade.
...
|:---|:---|
| **CLI** (`aimtutor`) | Typer-based CLI with sub-commands: `run`, `chat`, `bot`, `kb`, `memory`, `session`, `notebook`, `plugin`, `config`, `provider`, `serve` |
...
| **Python SDK** (`aimtutor.app.facade`) | Programmatic facade for SDK-style integrations |
...
New autonomous bot system (`aimtutor/tutorbot/`) that brings AIMTutor to messaging platforms:
...
- **Package layout:** `src/` → `aimtutor/` + `aimtutor_cli/`. Old `src/` directory fully removed (140 files).
- **Package renamed:** `ai-tutor` → `aimtutor`, version `1.0.0`.
...
- Complete codebase rewrite with agent-native architecture (AIMTutor 2.0).
- Two-layer plugin model (Tools + Capabilities) with `ChatOrchestrator` coordinator.
- Standalone CLI package (`aimtutor_cli/`) with 11 sub-commands via Typer.
...
**Full Changelog**: https://github.com/HKUDS/AIMTutor/compare/v0.6.0...v1.0.0-beta1
