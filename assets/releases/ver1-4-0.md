...
isolation hardening, and the `aimtutor start` / `aimtutor init` launcher)
...
- **`build_provider_extra_kwargs`** in `aimtutor/core/agentic/client.py`
...
The `aimtutor chat` REPL surface was tightened so it matches the public
docs at [aimtutor.info](https://aimtutor.info).

- `aimtutor chat --config key=value` and `--config-json '<json>'` seed
...
- `aimtutor plugin info <name>` now also returns `cli_aliases` and the
...
- `.gitignore` covers `.playwright-cli/`; stale `.playwright-cli/`
  captures, `.env.example_CN`, and `AIMTutor.code-workspace` removed.
...
- New `tests/cli/test_docs_contract.py` keeps the public site docs in
  sync with the CLI surface (links + `aimtutor …` examples).
- `tests/scripts/test_docker_compose.py` + `web/tests/api-resolve-base.test.ts`
  cover the new placeholder detection.

## Upgrade Notes
- **From v1.4.0-beta:** drop-in. PyPI installs with `pip install -U aimtutor`
...
**Full Changelog**: https://github.com/HKUDS/AIMTutor/compare/v1.4.0-beta...v1.4.0
