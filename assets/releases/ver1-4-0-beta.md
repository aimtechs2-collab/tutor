# AIMTutor v1.4.0-beta Release Notes
...
maze and make `aimtutor start` / `aimtutor init` first-class.
...
- **`aimtutor/runtime/banner.py`** — localized startup banner shared
  between `aimtutor start` and `aimtutor init`; reads the language
  preference from interface settings so the banner matches the UI locale.
- **`init_wizard.py`** — interactive `aimtutor init` wizard with provider
...
- **`aimtutor start` is the recommended launcher** — `start_web.py` /
  `start_tour.py` continue to work but are now thin wrappers around the
  new `runtime/launcher.py`. Run `aimtutor init` once to seed providers
...
**Full Changelog**: https://github.com/HKUDS/AIMTutor/compare/v1.3.10...v1.4.0-beta
