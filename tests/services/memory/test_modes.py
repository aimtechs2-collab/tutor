...
from aimtutor.services.memory import paths as paths_mod
from aimtutor.services.memory.consolidator.modes import audit as audit_mod
from aimtutor.services.memory.consolidator.modes import dedup as dedup_mod
from aimtutor.services.memory.consolidator.modes import update as update_mod
from aimtutor.services.memory.document import Document, Entry, parse, serialize
from aimtutor.services.memory.ids import new_entry_id
from aimtutor.services.memory.snapshot.entity import Entity
...
async def test_update_l2_appends_facts_from_chunk(memory_dir, monkeypatch):
    ...
    monkeypatch.setattr(
        "aimtutor.services.memory.consolidator.modes.update.snap.read_snapshot",
    ...
    with (
        patch("aimtutor.services.memory.consolidator.modes.update.call_llm", side_effect=fake_llm),
        patch.object(update_mod, "load_memory_settings") as mock_settings,
    ):
        from aimtutor.services.memory.settings import (
            ChunkingSettings,
            DedupSettings,
            MemorySettings,
        )
...
async def test_update_l2_idempotent_when_no_new_entities(memory_dir, monkeypatch):
    ...
    with (
        patch(
            "aimtutor.services.memory.consolidator.modes.update.call_llm",
            side_effect=llm_returns_one,
        ),
        patch.object(update_mod, "load_memory_settings") as mock_settings,
    ):
        from aimtutor.services.memory.settings import DedupSettings, MemorySettings
...
async def test_audit_l2_applies_replace_edit(memory_dir, monkeypatch):
    ...
    monkeypatch.setattr(
        "aimtutor.services.memory.consolidator.modes.audit.snap.read_snapshot",
    ...
    with patch("aimtutor.services.memory.consolidator.modes.audit.call_llm", side_effect=fake_llm):
...
async def test_dedup_early_stop_when_no_edits(memory_dir, monkeypatch):
    ...
    with (
        patch("aimtutor.services.memory.consolidator.modes.dedup.call_llm", side_effect=fake_llm),
