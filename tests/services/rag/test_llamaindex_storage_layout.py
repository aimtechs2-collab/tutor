...
from aimtutor.services.rag.index_versioning import EmbeddingSignature
...
async def test_incremental_add_migrates_matching_legacy_index_to_flat_version(
    ...
    from aimtutor.services.rag.pipelines.llamaindex import storage as storage_module
    from aimtutor.services.rag.pipelines.llamaindex.pipeline import LlamaIndexPipeline
...
def test_hybrid_retriever_uses_official_query_fusion_when_bm25_available(
    ...
    from aimtutor.services.rag.pipelines.llamaindex import retrievers as retriever_module
    from aimtutor.services.rag.pipelines.llamaindex.config import RetrievalConfig
...
def test_retrieval_config_reads_profile_from_env(monkeypatch: pytest.MonkeyPatch) -> None:
    from aimtutor.services.rag.pipelines.llamaindex import config as config_module

    monkeypatch.setenv("AIMTUTOR_RAG_RETRIEVAL_PROFILE", " vector ")
