...
from aimtutor.services.llm.factory import complete, stream
from aimtutor.services.llm.provider_core.base import LLMResponse
...
async def test_complete_merges_config_and_caller_extra_headers(monkeypatch) -> None:
    ...
    monkeypatch.setattr("aimtutor.services.llm.factory.get_llm_config", lambda: cfg)
    ...
    monkeypatch.setattr(
        "aimtutor.services.llm.factory.get_runtime_provider", _fake_get_runtime_provider
    )
...
async def test_explicit_call_inherits_matching_profile_headers_and_reasoning(
    ...
    cfg = _make_cfg(
        extra_headers={"User-Agent": "AIMTutor-Test"},
    ...
    assert result == "ok"
    assert captured_config["config"].extra_headers == {"User-Agent": "AIMTutor-Test"}
...
async def test_stream_does_not_replay_reasoning_as_final_content(monkeypatch) -> None:
    ...
    monkeypatch.setattr("aimtutor.services.llm.factory.get_llm_config", lambda: cfg)
    monkeypatch.setattr(
        "aimtutor.services.llm.factory.get_runtime_provider",
