...
from aimtutor.tutorbot.channels.zulip import ZulipChannel, ZulipConfig
...
class TestDownloadAttachments:
    def test_extracts_from_markdown_content(self, tmp_path: Path):
        ...
        with (
            ...
            patch(
                "aimtutor.tutorbot.channels.zulip.get_media_dir",
                return_value=tmp_path,
            ),
            patch("aimtutor.tutorbot.channels.zulip.requests.get") as mock_get,
        ):
            mock_resp = MagicMock()
...
class TestSubscribeStreams:
    ...
    def test_subscribe_partial_failure_logs_warning(self):
        ...
        with patch("aimtutor.tutorbot.channels.zulip.logger.warning") as mock_warning:
            ch._subscribe_to_streams()
    ...
    def test_subscribe_non_success_all_already_subscribed_is_debug_only(self):
        ...
        with (
            patch("aimtutor.tutorbot.channels.zulip.logger.warning") as mock_warning,
            patch("aimtutor.tutorbot.channels.zulip.logger.debug") as mock_debug,
        ):
            ch._subscribe_to_streams()
...
class TestStart:
    ...
    async def test_start_profile_failure(self, monkeypatch):
        ...
        with patch("aimtutor.tutorbot.channels.zulip.ZulipChannel._call_with_retry") as mock_retry:
