...
from aimtutor.services.tutorbot.manager import BotConfig, TutorBotInstance, TutorBotManager
from aimtutor.services.tutorbot.model_runtime import resolve_tutorbot_llm_config
...
class TestTutorBotModelRuntime:
    def test_selection_is_resolved_through_model_selection_service(
        ...
        monkeypatch.setattr(
            "aimtutor.services.tutorbot.model_runtime.resolve_llm_config_for_selection",
            fake_resolve,
        )
...
def test_start_bot_passes_shared_memory_dir(
    ...
    monkeypatch.setattr("aimtutor.tutorbot.agent.loop.AgentLoop", FakeAgentLoop)
    monkeypatch.setattr(
        "aimtutor.tutorbot.providers.aimtutor_adapter.create_aimtutor_provider",
        lambda *_args, **_kwargs: object(),
    )
    monkeypatch.setattr(
        "aimtutor.services.tutorbot.model_runtime.resolve_tutorbot_llm_config",
        lambda _cfg: SimpleNamespace(model="selected-model", context_window=123456),
    )
    monkeypatch.setattr(
        "aimtutor.services.tutorbot.manager.TutorBotManager._build_channel_manager",
        lambda *_args, **_kwargs: None,
    )
    monkeypatch.setattr(
        "aimtutor.services.tutorbot.manager.TutorBotManager._outbound_router",
        lambda *_args, **_kwargs: _done(),
    )
    monkeypatch.setattr("aimtutor.tutorbot.heartbeat.HeartbeatService", FakeHeartbeat)
