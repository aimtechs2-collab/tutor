...
from aimtutor.services.config.provider_runtime import ResolvedSearchConfig
from aimtutor.services.search import web_search
from aimtutor.services.search.types import WebSearchResponse
...
def test_web_search_rejects_deprecated_provider(monkeypatch) -> None:
    monkeypatch.setattr(
        "aimtutor.services.search._get_web_search_config",
        lambda: {"enabled": True},
    )
    monkeypatch.setattr(
        "aimtutor.services.search.resolve_search_runtime_config",
...
def test_web_search_perplexity_missing_key_hard_fails(monkeypatch) -> None:
    ...
    monkeypatch.setattr("aimtutor.services.search._resolve_provider_key", lambda _p, _k: "")
...
def test_web_search_missing_key_falls_back_to_duckduckgo(monkeypatch) -> None:
    ...
    monkeypatch.setattr("aimtutor.services.search._resolve_provider_key", lambda _p, _k: "")
    monkeypatch.setattr("aimtutor.services.search.get_provider", _fake_get_provider)
