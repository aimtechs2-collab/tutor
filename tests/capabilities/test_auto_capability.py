...
from aimtutor.capabilities.auto import AutoCapability
from aimtutor.capabilities.request_contracts import (
    AutoRequestConfig,
...
from aimtutor.services.session.turn_runtime import _should_capture_assistant_content
...
def fake_registry():
    ...
    from aimtutor.runtime.registry.capability_registry import CapabilityRegistry
...
def mock_llm():
    ...
    with patch(
        "aimtutor.agents.auto.auto_pipeline.get_llm_config",
...
def _install_scripted_client(client: _ScriptedClient):
    return patch.object(
        __import__("aimtutor.agents.auto.auto_pipeline", fromlist=["AutoPipeline"]).AutoPipeline,
...
async def test_single_delegation_happy_path(fake_registry, mock_llm):
    ...
    with (
        ...
        patch(
            "aimtutor.agents.auto.auto_pipeline.get_capability_registry",
            return_value=fake_registry,
        ),
        patch(
            "aimtutor.agents.auto.delegation.get_capability_registry",
            return_value=fake_registry,
        ),
    ):
...
async def test_atomic_tool_dispatch_with_rag_kb_injection(fake_registry, mock_llm):
    ...
    with (
        ...
        patch(
            "aimtutor.agents.auto.auto_pipeline.get_tool_registry",
