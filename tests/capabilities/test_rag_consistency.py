...
async def test_deep_solve_omits_rag_when_no_knowledge_base() -> None:
    ...
    with (
        patch(
            "aimtutor.capabilities.deep_solve.SolvePipeline",
            new=_FakePipeline,
        ),
        patch(
            "aimtutor.services.llm.config.get_llm_config",
...
async def test_deep_research_forwards_enabled_tools_and_kb_unchanged() -> None:
    ...
    with (
        patch(
            "aimtutor.capabilities.deep_research.ResearchPipeline",
            new=_FakePipeline,
        ),
        patch(
            "aimtutor.services.llm.config.get_llm_config",
        ...
        patch(
            "aimtutor.capabilities.deep_research.load_config_with_main",
            return_value={},
        ),
    ):
        await _drain(bus, capability.run(context, bus))
