...
``aimtutor.capabilities._answer_now.llm_stream`` (the alias the helper
imports) and ``aimtutor.capabilities._answer_now.get_llm_config`` so
...
from aimtutor.capabilities import _answer_now
from aimtutor.capabilities._answer_now import (
...
class TestChatAnswerNow:
    ...
    async def test_chat_synthesises_final_answer_from_partial_trace(self) -> None:
        from aimtutor.agents.chat import agentic_pipeline as ap_module
        ...
        with (
            ...
            patch(
                "aimtutor.agents.chat.agentic_pipeline.get_llm_config",
                return_value=cfg,
            ),
        ):
            from aimtutor.capabilities.chat import ChatCapability
...
class TestVisualizeAnswerNow:
    ...
    async def test_emits_renderable_code_envelope(self) -> None:
        ...
        with (
            ...
            from aimtutor.capabilities.visualize import VisualizeCapability
...
class TestMathAnimatorAnswerNow:
    ...
    async def test_skips_analysis_design_summary_but_calls_codegen_and_render(
        ...
        from aimtutor.agents.math_animator.models import (
            GeneratedCode,
            RenderResult,
        )
        ...
        from aimtutor.capabilities.math_animator import MathAnimatorCapability

        analysis_agent_calls: list[Any] = []
        design_agent_calls: list[Any] = []
        summary_agent_calls: list[Any] = []

        with patch("aimtutor.agents.math_animator.pipeline.MathAnimatorPipeline") as PipelineCls:
...
class TestNormalPathWhenNoAnswerNow:
    ...
    async def test_deep_solve_without_payload_calls_pipeline(self) -> None:
        from aimtutor.capabilities.deep_solve import DeepSolveCapability
        ...
        with (
            patch("aimtutor.capabilities.deep_solve.SolvePipeline") as PipelineCls,
            patch("aimtutor.services.llm.config.get_llm_config", return_value=_fake_llm_config()),
        ):
