...
from aimtutor.agents.chat.agentic_pipeline import AgenticChatPipeline
...
from aimtutor.core.tool_protocol import ToolResult
from aimtutor.core.trace import build_trace_metadata
...
async def test_execute_tool_call_streams_retrieve_progress_for_rag(
    ...
    monkeypatch.setattr(
        "aimtutor.agents.chat.agentic_pipeline.get_tool_registry", lambda: registry
    )
...
def _stub_optional_services(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "aimtutor.services.notebook.get_notebook_manager",
...
async def test_await_user_reply_overwrites_matching_tool_message(
    ...
    from aimtutor.agents.chat.agentic_pipeline import _DispatchOutcome
