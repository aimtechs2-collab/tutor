...
from aimtutor.agents.research.data_structures import DynamicTopicQueue, ToolTrace
from aimtutor.agents.research.pipeline import (
    LABEL_APPEND,
    LABEL_FINISH,
    LABEL_THINK,
    ResearchedBlock,
...
from aimtutor.core.agentic.tool_dispatch import DispatchOutcome
...
def _make_pipeline(monkeypatch: pytest.MonkeyPatch) -> ResearchPipeline:
    ...
    monkeypatch.setattr("aimtutor.agents.research.pipeline.get_llm_config", lambda: _FakeLLM())
    monkeypatch.setattr(
        "aimtutor.agents.research.pipeline.get_tool_registry", lambda: _FakeRegistry()
    )
...
def _make_pipeline_with_registry(
    ...
    *,
    ...
    monkeypatch.setattr("aimtutor.agents.research.pipeline.get_llm_config", lambda: _FakeLLM())
    monkeypatch.setattr("aimtutor.agents.research.pipeline.get_tool_registry", lambda: registry)
    monkeypatch.setattr("aimtutor.agents.research.pipeline.user_has_memory", lambda: False)
    monkeypatch.setattr("aimtutor.agents.research.pipeline.user_has_notebooks", lambda: False)
