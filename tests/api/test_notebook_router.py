...
notebook_router = importlib.import_module("aimtutor.api.routers.question_notebook").router
sessions_router = importlib.import_module("aimtutor.api.routers.sessions").router
...
def store(tmp_path: Path, monkeypatch) -> SQLiteSessionStore:
    ...
    monkeypatch.setattr(
        "aimtutor.api.routers.question_notebook.get_sqlite_session_store",
        lambda: instance,
    )
    monkeypatch.setattr(
        "aimtutor.api.routers.sessions.get_sqlite_session_store",
        lambda: instance,
    )
    return instance
