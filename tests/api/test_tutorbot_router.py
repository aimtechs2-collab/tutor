...
def _make_fake_manager(existing: dict | None = None):
    ...
    existing
        ...
        ``BotConfig``.
    """
    from aimtutor.services.tutorbot.manager import BotConfig
...
def _make_client(monkeypatch, existing: dict | None = None):
    ...
    tutorbot_router_mod = importlib.import_module("aimtutor.api.routers.tutorbot")
...
def _patch_llm_catalog(monkeypatch) -> None:
    config_mod = importlib.import_module("aimtutor.services.config")
    monkeypatch.setattr(
        config_mod,
