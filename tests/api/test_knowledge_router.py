...
if FastAPI is not None and TestClient is not None:
    knowledge_router_module = importlib.import_module("aimtutor.api.routers.knowledge")
...
def test_reindex_accepts_default_alias(monkeypatch, tmp_path: Path) -> None:
    ...
    embedding_signature = importlib.import_module("aimtutor.services.rag.embedding_signature")
    index_versioning = importlib.import_module("aimtutor.services.rag.index_versioning")
    monkeypatch.setattr(
...
def test_update_config_coerces_legacy_provider_to_llamaindex() -> None:
    ...
    config_module = importlib.import_module("aimtutor.services.config")
    app = _build_app()
