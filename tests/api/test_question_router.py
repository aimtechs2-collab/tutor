...
def _cleanup_question_router_module():
    yield
    sys.modules.pop("aimtutor.api.routers.question", None)
...
def _load_question_router_module(monkeypatch: pytest.MonkeyPatch):
    ...
    fake_agents = _package("aimtutor.agents")
    fake_agents_question = types.ModuleType("aimtutor.agents.question")
    fake_agents_question.AgentCoordinator = object
    fake_agents.question = fake_agents_question
    monkeypatch.setitem(sys.modules, "aimtutor.agents", fake_agents)
    monkeypatch.setitem(sys.modules, "aimtutor.agents.question", fake_agents_question)

    fake_logging = _package("aimtutor.logging")
    ...
    monkeypatch.setitem(sys.modules, "aimtutor.logging", fake_logging)

    fake_config = types.ModuleType("aimtutor.services.config")
    fake_config.PROJECT_ROOT = Path.cwd()
    fake_config.load_config_with_main = lambda *_args, **_kwargs: {}
    monkeypatch.setitem(sys.modules, "aimtutor.services.config", fake_config)

    fake_llm_package = _package("aimtutor.services.llm")
    fake_llm_config = types.ModuleType("aimtutor.services.llm.config")
    fake_llm_config.get_llm_config = lambda: None
    fake_llm_package.config = fake_llm_config
    monkeypatch.setitem(sys.modules, "aimtutor.services.llm", fake_llm_package)
    monkeypatch.setitem(sys.modules, "aimtutor.services.llm.config", fake_llm_config)

    fake_settings_package = _package("aimtutor.services.settings")
    fake_interface_settings = types.ModuleType("aimtutor.services.settings.interface_settings")
    ...
    monkeypatch.setitem(sys.modules, "aimtutor.services.settings", fake_settings_package)
    monkeypatch.setitem(
        sys.modules,
        "aimtutor.services.settings.interface_settings",
        fake_interface_settings,
    )

    fake_tools = _package("aimtutor.tools")
    fake_tools_question = types.ModuleType("aimtutor.tools.question")
    ...
    monkeypatch.setitem(sys.modules, "aimtutor.tools", fake_tools)
    monkeypatch.setitem(sys.modules, "aimtutor.tools.question", fake_tools_question)

    return importlib.import_module("aimtutor.api.routers.question")
