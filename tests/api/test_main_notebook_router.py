...
notebook_router = importlib.import_module("aimtutor.api.routers.notebook").router

from aimtutor.services.notebook.service import NotebookManager
...
def manager(tmp_path, monkeypatch) -> NotebookManager:
    ...
    monkeypatch.setattr(
        "aimtutor.api.routers.notebook.notebook_manager",
        instance,
    )
    return instance
...
def test_stream_add_record_with_summary_strips_thinking_tags(
    ...
    monkeypatch.setattr(
        "aimtutor.api.routers.notebook.NotebookSummarizeAgent",
        FakeSummarizeAgent,
    )
    nb = manager.create_notebook(name="My Notes")

    async def collect_events() -> list[dict]:
        request = importlib.import_module("aimtutor.api.routers.notebook").AddRecordRequest(
            notebook_ids=[nb["id"]],
            record_type="chat",
