...
from aimtutor.services.session.turn_runtime import TurnRuntimeManager
...
async def test_turn_runtime_replays_events_and_materializes_messages(
    ...
    monkeypatch.setattr("aimtutor.services.llm.config.get_llm_config", lambda: SimpleNamespace())
    monkeypatch.setattr(
        "aimtutor.services.session.context_builder.ContextBuilder", FakeContextBuilder
    )
    monkeypatch.setattr("aimtutor.runtime.orchestrator.ChatOrchestrator", FakeOrchestrator)
    monkeypatch.setattr(
        "aimtutor.book.context.build_book_context",
    ...
    monkeypatch.setattr(
        "aimtutor.services.memory.get_memory_store",
    ...
    monkeypatch.setattr(
        "aimtutor.services.skill.get_skill_service",
        _fake_skill_service,
    )
...
async def test_turn_runtime_persists_llm_selection_in_turn_snapshot(
    ...
    monkeypatch.setattr(
        "aimtutor.services.config.get_model_catalog_service",
        lambda: SimpleNamespace(load=_model_catalog),
    )
    monkeypatch.setattr(
        "aimtutor.services.model_selection.runtime.activate_llm_selection",
        fake_activate,
    )
    monkeypatch.setattr(
        "aimtutor.services.model_selection.runtime.reset_llm_selection",
    ...
    monkeypatch.setattr("aimtutor.services.skill.get_skill_service", _fake_skill_service)
