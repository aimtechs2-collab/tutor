...
import aimtutor.agents.visualize.pipeline as visualize_pipeline
from aimtutor.capabilities.chat import ChatCapability
from aimtutor.capabilities.deep_question import DeepQuestionCapability
from aimtutor.capabilities.deep_research import DeepResearchCapability
...
async def test_chat_capability_streams_content_and_geogebra_context(
    ...
    monkeypatch.setattr("aimtutor.capabilities.chat.AgenticChatPipeline", FakePipeline)
...
async def test_deep_solve_capability_forwards_to_pipeline(
    ...
    monkeypatch.setattr("aimtutor.capabilities.deep_solve.SolvePipeline", FakePipeline)
...
async def test_deep_question_capability_uses_single_call_followup_agent(
    ...
    _install_module(
        monkeypatch,
        "aimtutor.agents.question.coordinator",
        AgentCoordinator=FakeCoordinator,
    )
    _install_module(
        monkeypatch,
        "aimtutor.agents.question.agents.followup_agent",
        FollowupAgent=FakeFollowupAgent,
    )
    _install_module(
        monkeypatch,
        "aimtutor.services.llm.config",
...
async def test_deep_research_capability_delegates_to_pipeline(
    ...
    """
    import aimtutor.agents.research.request_config  # noqa: F401
    import aimtutor.capabilities.deep_research as deep_research_mod
