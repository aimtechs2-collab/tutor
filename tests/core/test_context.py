...
from aimtutor.core.errors import (
    ConfigurationError,
    AIMTutorError,
...
class TestErrors:
    def test_base_error_str_without_details(self) -> None:
        err = AIMTutorError("boom")
        assert str(err) == "boom"
        assert err.details == {}

    def test_base_error_str_with_details(self) -> None:
        err = AIMTutorError("boom", details={"key": "val"})
        assert "key" in str(err)

    def test_hierarchy(self) -> None:
        assert issubclass(ConfigurationError, AIMTutorError)
        assert issubclass(ValidationError, AIMTutorError)
        assert issubclass(ServiceError, AIMTutorError)
