...
from aimtutor.runtime.registry.tool_registry import ToolRegistry
from aimtutor.tools.builtin import (
    BrainstormTool,
    CodeExecutionTool,
...
async def test_brainstorm_tool_passes_llm_arguments(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install_module(monkeypatch, "aimtutor.tools.brainstorm", brainstorm=fake_brainstorm)
...
async def test_rag_tool_forwards_query_and_extra_kwargs(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install_module(monkeypatch, "aimtutor.tools.rag_tool", rag_search=fake_rag_search)
...
async def test_web_search_tool_wraps_sync_function(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install_module(monkeypatch, "aimtutor.tools.web_search", web_search=fake_web_search)
...
async def test_code_execution_tool_uses_direct_code_path(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install_module(monkeypatch, "aimtutor.tools.code_executor", run_code=fake_run_code)
...
async def test_reason_tool_passes_llm_arguments(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install_module(monkeypatch, "aimtutor.tools.reason", reason=fake_reason)
...
async def test_paper_search_tool_formats_papers(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install_module(
        monkeypatch,
        "aimtutor.tools.paper_search_tool",
...
async def test_geogebra_analysis_tool_handles_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install_module(
        monkeypatch,
        "aimtutor.agents.vision_solver.vision_solver_agent",
