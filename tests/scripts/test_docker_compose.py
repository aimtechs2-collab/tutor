...
def test_render_docker_env_reads_json_only(tmp_path: Path) -> None:
    ...
    assert values == {
        "AIMTUTOR_DOCKER_BACKEND_PORT": "9001",
        "AIMTUTOR_DOCKER_FRONTEND_PORT": "4000",
        "AIMTUTOR_DOCKER_POCKETBASE_PORT": "19090",
    }
    saved = output_path.read_text(encoding="utf-8")
    assert "\nBACKEND_PORT=" not in saved
    assert "AIMTUTOR_DOCKER_BACKEND_PORT=9001" in saved
...
def test_render_docker_env_uses_defaults_for_missing_or_invalid_json(tmp_path: Path) -> None:
    ...
    assert values["AIMTUTOR_DOCKER_BACKEND_PORT"] == "8001"
    assert values["AIMTUTOR_DOCKER_FRONTEND_PORT"] == "3782"
    assert values["AIMTUTOR_DOCKER_POCKETBASE_PORT"] == "8090"
...
def test_compose_files_do_not_consume_legacy_env_names() -> None:
    ...
    for name in ("docker-compose.yml", "docker-compose.ghcr.yml"):
        ...
        assert "AIMTUTOR_DOCKER_BACKEND_PORT" in content
...
def test_dockerfile_uses_runtime_auth_placeholder() -> None:
    ...
    assert "AIMTUTOR_IGNORE_PROCESS_ENV_OVERRIDES=1" in content
