...
def _aimtutor_commands() -> list[str]:
    commands: list[str] = []
    pending = ""
    for path in DOCS_ROOT.rglob("*.md"):
        for line in path.read_text(encoding="utf-8").splitlines():
            stripped = line.strip()
            if not pending and not stripped.startswith("aimtutor "):
                continue
...
def test_documented_aimtutor_subcommands_exist() -> None:
    top_level = {
        "bot",
        "book",
        "chat",
    ...
    for command in _aimtutor_commands():
...
def test_deep_research_examples_include_required_config() -> None:
    examples = [
        command for command in _aimtutor_commands() if "aimtutor run deep_research" in command
    ]
...
def test_docs_do_not_advertise_removed_cli_forms() -> None:
    ...
    assert "aimtutor provider logout" not in text
    assert "aimtutor memory show summary" not in text
