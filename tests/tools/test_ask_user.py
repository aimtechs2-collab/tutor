...
Covers both the v2 multi-question schema and the legacy single-question
shorthand which auto-wraps into a one-element list.
"""

from __future__ import annotations

from aimtutor.tools.ask_user import (
    MAX_OPTION_CHARS,
    MAX_OPTIONS,
    MAX_QUESTION_CHARS,
    MAX_QUESTIONS,
...
def test_legacy_options_strip_and_dedupe() -> None:
    ...
    assert err is None
    assert payload is not None
    assert payload.questions[0].options == ("A", "B", "C")
...
def test_legacy_clips_oversized_option() -> None:
    ...
    assert payload is not None
    assert payload.questions[0].options[0].endswith("…")
...
def test_to_dict_shape_is_v2_for_frontend() -> None:
    payload, _ = build_ask_user_payload(
        questions=[
            {"id": "scope", "prompt": "Q1", "options": ["a"]},
            {"id": "depth", "prompt": "Q2"},
        ],
        intro="hi",
    )
...
def test_to_dict_legacy_path_also_emits_v2() -> None:
    ...
    assert d["questions"][0]["options"] == ["a", "b"]
