...
function getTraceHeader(
  ...
  if (
    ...
    // top-level status row carries the verbose "AIMTutor Reflecting…"
...
function detectStreamingMode(
  ...
  for (let idx = events.length - 1; idx >= 0; idx -= 1) {
    ...
    if (event.type === "content" && callKind === "llm_final_response") {
      ...
      // "AIMTutor Exploring…" until the bus moves on.
      if (event.stage === "exploring") return "exploring";
    ...
    (mode === "tool_using"
      ...
      : mode === "planning"
        ...
        : mode === "drafting"
          ? t("AIMTutor Drafting…")
          : mode === "responding"
            ? t("AIMTutor Responding…")
            : mode === "exploring"
              ? t("AIMTutor Exploring…")
              : mode === "quizzing"
                ? t("AIMTutor Quizzing…")
                : mode === "reflecting"
                  ? t("AIMTutor Reflecting…")
                  : mode === "responded"
                    ? t("AIMTutor responded.")
                    : t("AIMTutor Reasoning…"));
