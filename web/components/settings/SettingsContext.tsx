...
type SettingsContextValue = {
  ...
  syncLlmModels: (profileId?: string | null) => Promise<void>;
  syncingLlmModels: boolean;
...
export function SettingsProvider({ children }: { children: ReactNode }) {
  ...
  const syncLlmModels = useCallback(
    async (profileId?: string | null) => {
      if (!catalogEditable) return;
      setSyncingLlmModels(true);
      try {
        const response = await apiFetch(apiUrl("/api/v1/settings/llm/models/sync"), {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            catalog: draft,
        ...
        const payload = (await response.json()) as {
          ...
          errors?: { profile_id?: string; message?: string }[];
          detail?: string;
        };
        if (!response.ok || !payload.catalog) {
          throw new Error(payload.detail || t("Could not sync models."));
        }
        setDraft(cloneCatalog(payload.catalog));
        const syncedModels = payload.synced_models ?? 0;
        const errors = payload.errors ?? [];
        if (errors.length > 0 && syncedModels === 0) {
          setToast(errors.map((error) => error.message).filter(Boolean).join("; "));
        } else if (errors.length > 0) {
          setToast(
            t("Synced {{count}} models; {{errors}} profile(s) need attention.", {
              count: syncedModels,
              errors: errors.length,
            }),
          );
        } else {
          setToast(t("Synced {{count}} models", { count: syncedModels }));
        }
      } catch (error) {
        const message =
          error instanceof Error ? error.message : t("Could not sync models.");
        setToast(message);
      } finally {
        setSyncingLlmModels(false);
      }
    },
