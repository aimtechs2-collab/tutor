  ...
  return (
    <div data-tour={`tour-${service}`} className="space-y-5">
      {activeProfile ? (
                ...
                <div className="mb-4 flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    {service === "llm" && (
                      <button
                        type="button"
                        onClick={() => syncLlmModels()}
                        disabled={
                          syncingLlmModels ||
                          !draft.services.llm.profiles.some((profile) =>
                            String(profile.api_key || "").trim(),
                          )
                        }
                        title={t(
                          "Fetch available models from every configured LLM provider.",
                        )}
        ...
        <div className="rounded-xl border border-dashed border-[var(--border)] px-5 py-12 text-center">
          ...
          <button
            type="button"
            onClick={() => addProfile(service)}
            className="mt-4 inline-flex items-center gap-1.5 rounded-lg border border-[var(--border)]/60 px-3 py-1.5 text-[12px] font-medium text-[var(--foreground)] transition-colors hover:border-[var(--border)] hover:bg-[var(--muted)]/30"
          >
      ...
      <div className="sm:col-span-2">
        ...
        {service === "embedding" && (
          <p className="mt-1.5 text-[11px] text-[var(--muted-foreground)]">
            {t(
              "Embedding requests are sent to this URL exactly; AIMTutor does not append /embeddings or /api/embed at request time.",
            )}
          </p>
        )}
      </div>
