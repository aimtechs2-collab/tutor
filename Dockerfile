# ============================================
# AIMTutor Multi-Stage Dockerfile
# ============================================
# This Dockerfile builds a production-ready image for AIMTutor
# containing both the FastAPI backend and Next.js frontend
#
# Build/run:
#   docker build -t aimtutor:local .
#   docker run -p 127.0.0.1:3782:3782 -p 127.0.0.1:8001:8001 \
#     -v aimtutor-data:/app/data aimtutor:local
#
# Prerequisites:
...
COPY aimtutor/__version__.py /app/aimtutor/__version__.py
...
# Labels
LABEL maintainer="AIMTutor Team" \
      description="AIMTutor: AI-Powered Personalized Learning Assistant"
...
ENV PYTHONDONTWRITEBYTECODE=1 \
    ...
    PYTHONIOENCODING=utf-8 \
    NODE_ENV=production \
    AIMTUTOR_IGNORE_PROCESS_ENV_OVERRIDES=1
...
COPY aimtutor/ ./aimtutor/
COPY aimtutor_cli/ ./aimtutor_cli/
...
RUN cat > /etc/supervisor/conf.d/aimtutor.conf <<'EOF'
[supervisord]
nodaemon=true
logfile=/dev/null
logfile_maxbytes=0
...
RUN sed -i 's/\r$//' /etc/supervisor/conf.d/aimtutor.conf
...
exec python -m uvicorn aimtutor.api.main:app --host 0.0.0.0 --port ${BACKEND_PORT}
EOF
...
echo "============================================"
echo "🚀 Starting AIMTutor"
echo "============================================"

export AIMTUTOR_IGNORE_PROCESS_ENV_OVERRIDES=1
...
eval "$(python - <<'PY'
import shlex
from aimtutor.services.config import export_runtime_settings_to_env
...
# Start supervisord
exec /usr/bin/supervisord -c /etc/supervisor/conf.d/aimtutor.conf
EOF
...
[program:backend]
command=python -m uvicorn aimtutor.api.main:app --host 0.0.0.0 --port %(ENV_BACKEND_PORT)s --reload
directory=/app
autostart=true
autorestart=true
stdout_logfile=/dev/fd/1
