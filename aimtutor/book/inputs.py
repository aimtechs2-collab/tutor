...
Reuses:
- ``aimtutor.services.session.get_sqlite_session_store`` for chat history
- ``aimtutor.services.notebook.notebook_manager`` for notebook records
- ``aimtutor.agents.notebook.NotebookAnalysisAgent`` for context distillation
"""
...
async def _resolve_notebook_context(
    ...
    *,
    ...
    if not notebook_refs:
        return "", 0
    try:
        from aimtutor.services.notebook import notebook_manager
