"""
Shared prompt loader for non-BaseAgent call sites in ``aimtutor/book``.

All LLM-facing prompts inside this module live as YAML under
``aimtutor/book/prompts/{en,zh}/<name>.yaml`` and are loaded through the
unified :class:`~aimtutor.services.prompt.PromptManager`. This helper is the
...
def load_book_prompts(name: str, language: str) -> dict[str, Any]:
    ...
    Args:
        name: File stem under ``aimtutor/book/prompts/{lang}/``
    ...
    if not prompts:
        raise RuntimeError(
            ...
            f"Expected aimtutor/book/prompts/{{en,zh}}/{name}.yaml."
        )
    return prompts
