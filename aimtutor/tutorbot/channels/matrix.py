...
try:
    ...
    raise ImportError(
        "Matrix dependencies not installed. Run: pip install aimtutor[matrix]"
    ) from e
...
from aimtutor.tutorbot.config.paths import get_data_dir, get_media_dir
from aimtutor.tutorbot.config.schema import Base
from aimtutor.tutorbot.utils.helpers import safe_filename
    ...
    async def start(self) -> None:
        ...
        if self.config.e2ee_enabled and not MATRIX_E2EE_AVAILABLE:
            raise ImportError(
                ...
                "`aimtutor[matrix-e2e]` or `requirements/matrix-e2e.txt`, and make "
    ...
    def _decrypt_media_bytes(self, event: MatrixMediaEvent, ciphertext: bytes) -> bytes | None:
        if decrypt_attachment is None:
            logger.warning(
                ...
                "Install aimtutor[matrix-e2e] to decrypt encrypted media."
            )
            return None
        key_obj, hashes, iv = (
