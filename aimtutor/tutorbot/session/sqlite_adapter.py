"""SessionManager adapter backed by AIMTutor's SQLite store.

Implements the SessionManager interface (get_or_create, save, list_sessions) but
reads/writes through AIMTutor's SQLiteSessionStore, unifying conversation history
for TutorBot and AIMTutor in a single database.
"""
...
from aimtutor.tutorbot.session.manager import Session


class SQLiteSessionAdapter:
    """Drop-in replacement for SessionManager, backed by AIMTutor SQLite."""

    def __init__(self, store: Any) -> None:
        """
        Args:
            store: A AIMTutor SQLiteSessionStore instance.
        """
        self.store = store
    ...
    def _session_id(self, key: str) -> str:
        """Derive a stable AIMTutor session_id from a TutorBot key (channel:chat_id)."""
    ...
    def _ensure_sqlite_session_sync(self, key: str) -> None:
        """Ensure a corresponding AIMTutor session row exists."""
        session_id = self._session_id(key)
        try:
            loop = asyncio.get_running_loop()
        except RuntimeError:
