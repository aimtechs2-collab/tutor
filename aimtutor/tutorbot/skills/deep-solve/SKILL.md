---
name: deep-solve
description: "Multi-stage problem solving via AIMTutor (plan → reason → write)."
...
Use the `exec` tool to invoke AIMTutor's multi-stage solver for complex math, science, or engineering problems.
...
```bash
aimtutor run deep_solve "<problem description>" --format json -l <lang>
```
...
Prove a theorem:
```bash
aimtutor run deep_solve "Prove that for any positive integer n, n³ - n is divisible by 6" --format json -l en
```

Solve with knowledge base:
```bash
aimtutor run deep_solve "Derive the Euler-Lagrange equation" --format json -l en -t rag --kb physics-textbook
```

Solve with code verification:
```bash
aimtutor run deep_solve "Find all roots of x⁴ - 5x² + 4 = 0" --format json -l en -t code_execution
```
