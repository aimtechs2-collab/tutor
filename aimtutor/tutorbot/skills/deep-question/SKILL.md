...
Use the `exec` tool to invoke AIMTutor's quiz generation pipeline (ideation → evaluation → generation → validation).
...
```bash
aimtutor run deep_question "<topic>" --format json -l <lang> [options]
```
...
Basic quiz:
```bash
aimtutor run deep_question "Calculus integration techniques" --format json -l en --config num_questions=5 --config difficulty=medium
```

Multiple-choice from a textbook:
```bash
aimtutor run deep_question "Chapter 3: Linear Algebra" --format json -l zh -t rag --kb math-textbook --config question_type=multiple_choice --config num_questions=10
```

Hard open-ended questions:
```bash
aimtutor run deep_question "Quantum mechanics fundamentals" --format json -l en --config difficulty=hard --config question_type=open_ended
```
