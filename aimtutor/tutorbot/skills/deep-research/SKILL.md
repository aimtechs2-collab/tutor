...
Use the `exec` tool to invoke AIMTutor's multi-agent research pipeline for comprehensive topic analysis.
...
```bash
aimtutor run deep_research "<topic>" --format json -l <lang> --config-json '<json>'
```
...
```bash
aimtutor run deep_research "Recent advances in attention mechanisms" --format json -l en --config-json '{"mode":"report","depth":"deep","sources":["papers","web"]}'
```

Quick comparison:
```bash
aimtutor run deep_research "Rust vs Go for backend services" --format json -l en --config-json '{"mode":"comparison","depth":"quick","sources":["web"]}'
```

Learning path from knowledge base:
```bash
aimtutor run deep_research "Machine learning fundamentals" --format json -l zh --config-json '{"mode":"learning_path","depth":"standard","sources":["kb"]}' --kb ml-textbook
```

KB + web hybrid:
```bash
aimtutor run deep_research "Agentic RAG vs traditional RAG" --format json -l zh --config-json '{"mode":"comparison","depth":"deep","sources":["kb","web"]}' --kb my-papers
```
