...
from aimtutor.core.stream import StreamEvent, StreamEventType
from aimtutor.services.llm.utils import clean_thinking_tags
from aimtutor.services.path_service import get_path_service
from aimtutor.services.session.protocol import SessionStoreProtocol
...
def _llm_selection_dict(value: Any) -> dict[str, str] | None:
    from aimtutor.services.model_selection import LLMSelection
...
class TurnRuntimeManager:
    ...
    async def start_turn(self, payload: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        try:
            from aimtutor.capabilities.request_contracts import validate_capability_config
        ...
        if llm_selection:
            try:
                from aimtutor.multi_user.model_access import apply_allowed_llm_selection
            ...
            from aimtutor.multi_user.model_access import redacted_model_access
        ...
        if llm_selection:
            ...
            from aimtutor.services.model_selection import (
                LLMSelection,
        ...
        if payload.get("tools") is None:
            try:
                from aimtutor.api.routers.settings import get_enabled_optional_tools
    ...
    async def _run_turn(self, execution: _TurnExecution) -> None:
        ...
        try:
            from aimtutor.agents.notebook import NotebookAnalysisAgent
            from aimtutor.book.context import build_book_context
            from aimtutor.core.context import Attachment, UnifiedContext
            from aimtutor.runtime.orchestrator import ChatOrchestrator
            from aimtutor.services.memory import get_memory_store
            from aimtutor.services.model_selection.runtime import (
                activate_llm_selection,
            )
            from aimtutor.services.model_selection.runtime import (
                reset_llm_selection as reset_active_llm_selection,
            )
            from aimtutor.services.notebook import get_notebook_manager
            from aimtutor.services.session.context_builder import ContextBuilder
            from aimtutor.services.skill import get_skill_service
            ...
            from aimtutor.services.storage import get_attachment_store
            ...
            from aimtutor.multi_user.context import get_current_user
            from aimtutor.multi_user.paths import get_admin_path_service
            from aimtutor.multi_user.skill_access import assigned_skill_ids
            ...
            if is_chat_capability:
                from aimtutor.services.session.source_inventory import (
                    build_inventory,
                    render_manifest,
                )
