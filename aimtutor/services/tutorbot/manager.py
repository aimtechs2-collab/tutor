...
Each TutorBot instance runs as a set of asyncio tasks within the AIMTutor
...
class TutorBotManager:
    ...
    async def start_bot(self, bot_id: str, config: BotConfig | None = None) -> TutorBotInstance:
        ...
        from aimtutor.services.tutorbot.model_runtime import (
        ...
        from aimtutor.tutorbot.config.schema import ExecToolConfig
        from aimtutor.tutorbot.providers.aimtutor_adapter import create_aimtutor_provider
        from aimtutor.tutorbot.session.manager import SessionManager

        llm_config = resolve_tutorbot_llm_config(config)
        provider = create_aimtutor_provider(llm_config)
        bus = MessageBus()
        ...
        # -- Heartbeat --------------------------------------------------------
        from aimtutor.tutorbot.heartbeat import HeartbeatService
    ...
    async def _outbound_router(self, bot_id: str, bus: Any, instance: TutorBotInstance) -> None:
        ...
        """
        try:
            from aimtutor.events.event_bus import Event, EventType, get_event_bus
            from aimtutor.tutorbot.bus.events import OutboundMessage as _OMsg
    ...
    def _build_channel_manager(
        ...
        *,
        ...
        from aimtutor.tutorbot.config.schema import ChannelsConfig
