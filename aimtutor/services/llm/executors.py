...
from aimtutor.services.llm.provider_registry import find_by_name, strip_provider_prefix
...
async def _create_with_format_fallback(
    ...
    *,
    ...
    try:
        ...
        if "temperature" in payload and is_unsupported_temperature_error(exc):
            logger.warning(
                "Provider %s rejected temperature for model %s (%s); retrying without it.",
                binding,
                model,
                exc,
            )
...
async def sdk_complete(
    ...
    **kwargs: Any,
    ...
    if supports_temperature(resolved_model, reasoning_effort):
        payload["temperature"] = temperature_val
