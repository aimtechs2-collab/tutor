...
from ..model_params import is_unsupported_temperature_error, supports_temperature
    ...
    async def complete(self, prompt: str, **kwargs: object) -> TutorResponse:
        ...
        async def _call_api() -> TutorResponse:
            ...
            try:
                ...
                if "temperature" not in request_kwargs or not is_unsupported_temperature_error(exc):
                    raise
                logger.warning(
                    "OpenAI rejected temperature for model %s; retrying without it.",
                    model,
                )
        ...
        async def _create_stream() -> OpenAIStream:
            ...
            try:
                ...
                if "temperature" not in request_kwargs or not is_unsupported_temperature_error(exc):
                    raise
                logger.warning(
                    "OpenAI rejected stream temperature for model %s; retrying without it.",
                    model,
                )
                retry_kwargs = dict(request_kwargs)
                retry_kwargs.pop("temperature", None)
            ...
            return cast(OpenAIStream, stream)
