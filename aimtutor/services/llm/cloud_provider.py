            ...
            async with session.post(url, headers=headers, json=data) as resp:
                if resp.status == 200:
                    ...
                    if (
                        ...
                        and is_unsupported_temperature_error(error_text)
                    ):
                        logger.warning(
                            "Provider %s rejected temperature for model %s "
        ...
        for retry_attempt in range(2):
            ...
            try:
                ...
                if (
                    retry_attempt == 0
                    and resp.status == 400
                    and "temperature" in attempt_data
                    and is_unsupported_temperature_error(error_text)
                ):
                    logger.warning(
                        "Provider %s rejected temperature for model %s "
                    ...
                    attempt_data.pop("temperature", None)
