...
from aimtutor.services.llm.openai_http_client import disable_ssl_verify_enabled
...
def _build_headers(account_id: Any, token: Any) -> dict[str, str]:
    if not token:
        raise RuntimeError(
            "OpenAI Codex is not logged in. Run `aimtutor provider login openai-codex`."
        )
    headers = {
        "Authorization": f"Bearer {token}",
        "OpenAI-Beta": "responses=experimental",
        "originator": DEFAULT_ORIGINATOR,
        "User-Agent": "AIMTutor (python)",
