...
from aimtutor.services.llm.capabilities import disable_response_format_at_runtime
from aimtutor.services.llm.model_params import supports_temperature
from aimtutor.services.llm.openai_http_client import openai_client_kwargs
from aimtutor.services.llm.provider_core.base import LLMProvider, LLMResponse, ToolCallRequest
from aimtutor.services.llm.provider_core.openai_responses import (
...
if TYPE_CHECKING:
    from aimtutor.services.provider_registry import ProviderSpec
...
_DEFAULT_OPENROUTER_HEADERS = {
    "HTTP-Referer": "https://github.com/HKUDS/AIMTutor",
    "X-OpenRouter-Title": "AIMTutor",
}
...
class OpenAICompatProvider(LLMProvider):
    ...
    @classmethod
    def _adapt_responses_extra_kwargs(
        cls,
        extra_kwargs: dict[str, Any],
        model_name: str,
        reasoning_effort: str | None,
    ) -> dict[str, Any]:
        adapted = adapt_chat_kwargs_to_responses(extra_kwargs)
        if not supports_temperature(model_name, reasoning_effort):
        ...
        try:
            if self._should_use_responses_api(model, reasoning_effort):
                try:
                    ...
                    body.update(
                        self._adapt_responses_extra_kwargs(
                            extra_kwargs,
                            str(body["model"]),
                            reasoning_effort,
                        )
