...
class ConfigTestRunner:
    ...
    async def _test_llm(self, run: TestRun, catalog: dict[str, Any]) -> None:
        from aimtutor.services.llm import clear_llm_config_cache, get_token_limit_kwargs
        from aimtutor.services.llm import complete as llm_complete
    ...
    async def _test_embedding(
        ...
        from aimtutor.services.embedding.client import EmbeddingClient
        from aimtutor.services.embedding.config import EmbeddingConfig
        ...
        probe_texts = [
            "AIMTutor embedding smoke test",
            "AIMTutor retrieval batch probe",
        ]
    ...
    def _test_search(self, run: TestRun, catalog: dict[str, Any]) -> None:
        from aimtutor.services.search import web_search
        ...
        run.emit("info", "Running search query: AIMTutor configuration health check")
        result = web_search("AIMTutor configuration health check", provider=provider)
        run.emit(
            "response",
