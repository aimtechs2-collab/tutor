...
from aimtutor.runtime.home import PACKAGE_ROOT
...
class PromptManager:
    ...
    # Modules that are not under aimtutor/agents/ directory
    # Map module_name → on-disk path component under aimtutor/
    ...
    def _candidate_prompt_dirs(self, module_name: str) -> list[Path]:
        ...
        if module_name in self.NON_AGENT_MODULES:
            ...
            current_dir = PACKAGE_ROOT / "aimtutor" / module_name / "prompts"
            return [legacy_dir, current_dir]

        legacy_dir = PACKAGE_ROOT / "src" / "agents" / module_name / "prompts"
        current_dir = PACKAGE_ROOT / "aimtutor" / "agents" / module_name / "prompts"
        return [legacy_dir, current_dir]
