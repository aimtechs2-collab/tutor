"""
Authentication service for AIMTutor.
...
from aimtutor.services.config import load_auth_settings, load_integrations_settings
...
if AUTH_ENABLED and not POCKETBASE_ENABLED and not AUTH_SECRET:
    from aimtutor.multi_user.identity import load_or_create_auth_secret
...
def _make_user_record(hashed: str, role: str = "user", created_at: str = "") -> dict[str, Any]:
    ...
    from aimtutor.multi_user.identity import new_user_id
...
def _load_users() -> dict[str, dict]:
    ...
    """
    from aimtutor.multi_user.identity import load_users
...
def delete_user(username: str) -> bool:
    ...
    """
    from aimtutor.multi_user.identity import delete_user as _delete_user
...
def set_role(username: str, role: str) -> bool:
    ...
    from aimtutor.multi_user.identity import set_role as _set_role
...
def decode_token(token: str) -> TokenPayload | None:
    ...
    if POCKETBASE_ENABLED:
        from aimtutor.services.pocketbase_client import validate_pb_token
...
def authenticate_pb(username: str, password: str) -> tuple[TokenPayload, str] | None:
    ...
    <username>@aimtutor.local to match the email used at registration.
    """
    try:
