...
from aimtutor.logging import configure_logging
from aimtutor.services.config import (
...
def validate_tool_consistency():
    ...
    """
    try:
        from aimtutor.runtime.registry.capability_registry import get_capability_registry
...
async def lifespan(app: FastAPI):
    ...
    try:
        from aimtutor.services.llm import get_llm_client
    ...
    try:
        from aimtutor.events.event_bus import get_event_bus
    ...
    try:
        from aimtutor.services.tutorbot import get_tutorbot_manager
    ...
    try:
        from aimtutor.services.pocketbase_client import ping_pocketbase
    ...
    try:
        from aimtutor.services.memory import migrate_v1_if_needed
...
app = FastAPI(
    ...
    # See: https://github.com/HKUDS/AIMTutor/issues/112
...
try:
    from aimtutor.services.setup import init_user_directories
...
from aimtutor.api.routers.presence import router as presence_router
from aimtutor.api.routers.clerk_webhooks import router as clerk_webhooks_router  # noqa: E402  # noqa: E402  # noqa: E402
...
app.include_router(
    ...
    prefix="/api/webhooks",
    tags=["webhooks"],
    # No auth dependency — Clerk signs requests with HMAC (svix)
)
app.include_router(
    presence_router,
...
@app.get("/")
async def root():
    return {"message": "Welcome to AIMTutor API"}


if __name__ == "__main__":
    from aimtutor.api.run_server import main as run_server_main
