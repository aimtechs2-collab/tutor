...
from aimtutor.api.utils.progress_broadcaster import ProgressBroadcaster
from aimtutor.api.utils.task_id_manager import TaskIDManager
from aimtutor.api.utils.task_log_stream import capture_task_logs, get_task_stream_manager
from aimtutor.knowledge.add_documents import DocumentAdder
from aimtutor.knowledge.initializer import KnowledgeBaseInitializer
from aimtutor.knowledge.manager import KnowledgeBaseManager
from aimtutor.knowledge.naming import validate_knowledge_base_name
from aimtutor.knowledge.progress_tracker import ProgressStage, ProgressTracker
from aimtutor.multi_user.context import get_current_user
from aimtutor.multi_user.knowledge_access import (
    assert_writable,
    current_kb_base_dir,
    current_kb_manager,
...
from aimtutor.services.rag.factory import DEFAULT_PROVIDER
from aimtutor.services.rag.file_routing import FileTypeRouter
from aimtutor.utils.document_validator import DocumentValidator
from aimtutor.utils.error_utils import format_exception_message
...
def _upload_file_to_pb(kb_name: str, filename: str, file_path: Path) -> None:
    ...
    try:
        from aimtutor.services.pocketbase_client import get_pb_client
...
def _matching_index_is_valid(kb_name: str, matching_version: dict | None) -> bool:
    ...
    if not matching_version:
        return False
    try:
        from aimtutor.services.rag.pipelines.llamaindex.storage import (
...
async def get_rag_providers():
    ...
    try:
        from aimtutor.services.rag.service import RAGService
...
async def get_all_kb_configs():
    ...
    try:
        from aimtutor.services.config import get_kb_config_service
...
async def reindex_knowledge_base(
    ...
    try:
        ...
        from aimtutor.services.rag.embedding_signature import signature_from_embedding_config
        from aimtutor.services.rag.index_versioning import (
...
async def websocket_progress(websocket: WebSocket, kb_name: str):
    ...
    from aimtutor.api.routers.auth import ws_auth_failed, ws_require_auth
    from aimtutor.multi_user.context import reset_current_user
    ...
    try:
        ...
        from aimtutor.services.rag.index_versioning import list_kb_versions
