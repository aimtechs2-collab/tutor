...
def _discover_plugins() -> list[Any]:
    try:
        from aimtutor.plugins.loader import discover_plugins
    except Exception:
...
async def _execute_stream(tool_name: str, params: dict[str, Any]) -> AsyncGenerator[str, None]:
    ...
    stdout_stream = _QueueTextStream(
        event_queue, loop, stream=None, logger_name="aimtutor.playground.stdout"
    )
    stderr_stream = _QueueTextStream(
        event_queue, loop, stream=None, logger_name="aimtutor.playground.stderr"
    )
