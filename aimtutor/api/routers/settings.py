...
from aimtutor.multi_user.model_access import allowed_llm_options, redacted_model_access
from aimtutor.services.config import get_config_test_runner, get_model_catalog_service
from aimtutor.services.embedding.client import reset_embedding_client
from aimtutor.services.llm.client import reset_llm_client
from aimtutor.services.llm.config import clear_llm_config_cache
...
from aimtutor.tools.builtin import USER_TOGGLEABLE_TOOL_NAMES
...
class SyncLlmModelsPayload(BaseModel):
    catalog: dict[str, Any]
...
def _provider_choices() -> dict[str, list[dict[str, str]]]:
    ...
    from aimtutor.services.config.provider_runtime import EMBEDDING_PROVIDERS
    from aimtutor.services.provider_registry import PROVIDERS
...
async def sync_llm_models_endpoint(payload: SyncLlmModelsPayload):
    _require_settings_admin()
    from aimtutor.services.config.llm_model_sync import sync_llm_models

    result = await sync_llm_models(payload.catalog, profile_id=payload.profile_id)
    return {
        "catalog": result.catalog,
        "synced_profiles": result.synced_profiles,
        "synced_models": result.synced_models,
...
async def complete_tour(payload: TourCompletePayload | None = None):
    ...
    return {
        "status": "completed",
        "message": "Configuration saved. AIMTutor will restart shortly.",
        "launch_at": launch_at,
