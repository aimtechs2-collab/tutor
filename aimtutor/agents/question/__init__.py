...
The main entry point is :class:`~aimtutor.agents.question.pipeline.QuestionPipeline`.
...
def __getattr__(name: str) -> Any:
    if name == "AgentCoordinator":
        module = import_module("aimtutor.agents.question.coordinator")
        return getattr(module, name)
    if name == "FollowupAgent":
        module = import_module("aimtutor.agents.question.agents.followup_agent")
        return getattr(module, name)
    if name in {"QuestionPipeline", "QuizTemplate", "QuizPair", "QuizPlan", "QuizHistoryEntry"}:
        module = import_module("aimtutor.agents.question.pipeline")
        return getattr(module, name)
