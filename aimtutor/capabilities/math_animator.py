...
from aimtutor.core.trace import build_trace_metadata, merge_trace_metadata, new_call_id
...
class MathAnimatorCapability(BaseCapability):
    ...
    async def run(self, context: UnifiedContext, stream: StreamBus) -> None:
        if importlib.util.find_spec("manim") is None:
            raise RuntimeError(
                ...
                "Install with `pip install 'aimtutor[math-animator]'` "
                "or `pip install -r requirements/math-animator.txt`."
            )
        from aimtutor.agents.math_animator.pipeline import MathAnimatorPipeline
        from aimtutor.agents.math_animator.request_config import (
    ...
    async def _run_answer_now(
        ...
        * **Keep** ``code_generation`` + ``render`` so the user still gets
        ...
        from aimtutor.agents.math_animator.models import ConceptAnalysis, SceneDesign
