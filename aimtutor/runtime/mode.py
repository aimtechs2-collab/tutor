...
Controls whether AIMTutor is running as a CLI application or an API server.
...
def _resolve_mode() -> RunMode:
    raw = os.environ.get("AIMTUTOR_MODE", "").strip().lower()
...
def set_mode(mode: RunMode) -> None:
    ...
    global _current_mode
    _current_mode = mode
    os.environ["AIMTUTOR_MODE"] = mode.value
