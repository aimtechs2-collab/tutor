"""Branded banner + localized labels for ``aimtutor start`` / ``aimtutor init``.
...
from aimtutor.__version__ import __version__

_ASCII_LOGO = r"""    _    ___ __  __ _____      _
...
LABELS: dict[str, dict[str, str]] = {
    "en": {
        ...
        "init.next_step": "Run `aimtutor start` to launch AIMTutor.",
        "init.choice": "Choice",
        ...
        "start.frontend_restart_failed": (
            ...
            "Stop PID {pid} and run `aimtutor start` again."
        ),
        ...
        "start.port_in_use": (
            "AIMTutor cannot start because port(s) already in use: {ports}. "
    ...
    "zh": {
        ...
        "init.next_step": "运行 `aimtutor start` 启动 AIMTutor。",
        "init.choice": "请选择",
        ...
        "start.frontend_restart_failed": (
            ...
            "请先停止 PID {pid},然后重新运行 `aimtutor start`。"
        ),
...
def render_banner(language: str | None, *, mode_key: str | None = None) -> Panel:
    ...
    return Panel(
        Align.left(body),
        title="[bold bright_cyan]AIMTutor[/]",
