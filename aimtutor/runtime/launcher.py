"""Local Web launcher for the installed AIMTutor app."""
...
from aimtutor.runtime.banner import labels_for, print_banner, resolve_language
...
def _safe_print(message: str) -> None:
    try:
        ...
        safe = message.encode(getattr(sys.stdout, "encoding", None) or "utf-8", errors="replace")
...
def _reset_runtime_singletons() -> None:
    """Make a just-selected AIMTUTOR_HOME visible to path/config singletons."""
    try:
        ...
        pass
    try:
        from aimtutor.services.config.runtime_settings import RuntimeSettingsService
...
def _stream_output(prefix: str, process: subprocess.Popen[str]) -> None:
    ...
    for line in process.stdout:
        _safe_print(f"  {prefix:<8} {line.rstrip()}")
...
def _packaged_web_dir() -> Path | None:
    try:
        ...
        return None
    path = Path(aimtutor_web.__file__).resolve().parent
...
def _copy_packaged_web_if_needed(
    ...
    *,
    ...
    marker = cache / ".aimtutor-web-runtime.json"
...
def _resolve_frontend(
    ...
    *,
    ...
    if packaged is not None:
        if not node:
            raise SystemExit("Node.js 20+ is required to run the packaged AIMTutor Web app.")
    ...
    raise SystemExit(
        "AIMTutor Web assets are not installed. Install the full app with `pip install -U aimtutor`, "
...
def start(home: str | Path | None = None) -> None:
    ...
    os.environ[AIMTUTOR_HOME_ENV] = str(runtime_home)
    ...
    common_env[AIMTUTOR_HOME_ENV] = str(runtime_home)
